...
from .forms import (
    ...
    UserLoginForm,
    UpdateUserForm,
    BuddyRegistrationForm,
    UserEmailPasswordResetForm,
)
...
def verify_user_and_activate(request, useremail, usertoken):
    try:
        ...
        messages.warning(
            request, "There is something wrong with this link... Verification failed"
        )
        return redirect("login")
        ...
        messages.warning(
            request, "This link was modified before verification.Faulty Link Detected!"
        )
        return redirect("login")
    except InvalidToken:
        messages.warning(
        ...
        if form.is_valid():
            ...
            for key, error in list(form.errors.items()):
                if key == "captcha" and error[0] == "This field is required.":
                    messages.error(
                        request, "You must pass the reCAPTCHA test to register."
                    )
                    continue
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for key, error in list(form.errors.items()):
                if key == "captcha" and error[0] == "This field is required.":
                    messages.error(
                        request, "You must pass the reCAPTCHA test to login."
                    )
                    return redirect("login")
                ...
                if (
                    ...
                    try:
                        ...
                        if not syukuser.is_verified and not userobj.is_active:
                            messages.error(
                                request,
                                "This email address is not verified! Please check your inbox and verify from there to access the content.",
                            )
                            return redirect("login")
                        messages.error(
                            request,
...
def sdp(request):
    ...
    if request.method == "POST":
        ...
        if int(form["amount"].value()) < 5 and form["frequency"].value() == "W":
            ...
            messages.success(
                request, "Click on the Paypal button below to complete your SDP!"
            )
            return render(
                request,
                "sdp.html",
...
def profile(request):
    ...
    if request.method == "POST":
        form = UpdateUserForm(
            ...
            user=request.user.startyoungukuser,
        )
        if form.is_valid():
            if (
                User.objects.filter(
                ...
                if len(request.FILES) != 0:
                    if form.cleaned_data["image"] and os.path.exists(
                        syuk_user.image.path
                    ):
                ...
                userx.save()
                messages.success(
                    request, f"Account updated successfully for {request.user}!"
                )
        ...
        form = UpdateUserForm(
            user=request.user.startyoungukuser, instance=request.user.startyoungukuser
        )
...
def past_donations(request):
    ...
    if request.method == "POST":
        ...
        response[
            "Content-Disposition"
        ] = f'attachment; filename="receipt_{donation.name}_{donation_date}.pdf"'
        buffer = BytesIO()
