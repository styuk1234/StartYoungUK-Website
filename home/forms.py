...
class CampaignForm(forms.ModelForm):
    ...
    campaign_description = forms.CharField(widget=forms.Textarea(), required=True)
    collection_target = forms.IntegerField(required=True)
    campaign_deadline = forms.DateField(
        required=True, help_text="Enter date in YYYY-MM-DD format"
    )
