...
def paypal_payment_received(sender, **kwargs):
    ...
    # check for subscription cancellation IPN
        ...
        buddy_id, user_id = int(ipn_obj.custom.split(" ")[1]), int(
            ipn_obj.custom.split(" ")[3]
        )
        try:
            if buddy_id != 0:
            ...
            if buddy_id != 0:
                ...
                sendEmailFixedContent(
                    ...
                    "email_templates/buddy_sdp_cancel.html",
                )
        ...
        print(
            "Paypal payment status: %s. Paypal transaction type: %s"
            % (ipn_obj.payment_status, ipn_obj.txn_type)
        )
